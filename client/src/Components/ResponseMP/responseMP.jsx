...
import {
	...
	getOrderMP,
	postOrderMP,
} from "../../redux/reducer/getResponseMP";
...
const ResponseMP = () => {
	...
	let paymentIdOrdenEl = useSelector(
		(state) => state.paymentOrderEG.paymentId
	);
	let getOrderMPEl = useSelector(
		(state) => state.paymentOrderEG.paymentOrder
	);
	let getOrderAllEl = useSelector(
		(state) => state.paymentOrderEG.OrdenCompra
	);
	useEffect(() => {
				...
				<div className="row mt-4">
					<div className="col-12 col-lg-12">
							...
							<div className="text-95 col-sm-6 align-self-start d-sm-flex justify-content-end">
								...
								<div className="text-grey-m2">
									...
									<div className="my-2">
										...
										{getOrderAllEl?.orderBd?.createdAt.substr(
											8,
											2
										)}
										/
						...
						<div className="row text-600 text-white bgc-default-tp1 py-25">
							...
							<div className="d-none d-sm-block col-2 col-sm-2">
								Cant.
							</div>
						...
						<div className="mt-4">
							<div className="text-95 text-secondary-d3">
								{getOrderMPEl?.shoppingCart?.map(
									(elem, index) => {
										// subTotal      = subTotal      + elem.subTotal
										totalPagar =
											totalPagar + elem.totalPage;
										// descuentoItem = descuentoItem + elem.discountItem
										return (
											<div
												key={elem.title}
												className="row mb-2 mb-sm-0 py-25"
											>
	...
	try {
		...
		await doc.html(pdfjs, {
			callback: function (doc) {
				doc.save(
					`OrdenCompra#${getOrderAllEl?.orderBd?.orderNumber}.pdf`
				);
			},
		}); //,
		//x: 10,y: 10});
